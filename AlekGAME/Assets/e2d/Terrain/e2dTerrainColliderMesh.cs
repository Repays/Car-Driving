...
public class e2dTerrainColliderMesh : e2dTerrainMesh
{
	...
	public void RebuildMesh()
	{
        ...
        for (int i = 0; i < polygon.Count; i += 30)
        {
            polygon.Add(new Vector2(polygon[i].x, min));
        }

        GameObject _collider = GameObject.Find("e2dTerrain");
        if (_collider)
        {
            PolygonCollider2D polygon2D = _collider.GetComponent<PolygonCollider2D>();
            if (!polygon2D) polygon2D = _collider.AddComponent<PolygonCollider2D>();
            polygon2D.points = polygon.ToArray();
        }
